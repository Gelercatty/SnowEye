...
class LoadWebcam:  # for inference
    def __init__(self, pipe='0', img_size=640, stride=32):
        ...
        self.cap.set(cv2.CAP_PROP_BUFFERSIZE, 100)  # set buffer size
...
class LoadStreams:  # multiple IP or RTSP cameras
    ...
    def update(self, index, cap):
        ...
        cont = 0
        while True:  # keep trying to read
            cont+=1
            # print("updating",cont)
            success,_ = cap.read()
            # print(success)
            try:
                if cap.isOpened():
                    ...
                    # print("loader",im)
                    if success:
                        # If frame is successfully read, store it as the latest frame
                        self.imgs[index] = im
                    else:
                        raise Exception(f"Error reading from stream {index}")
                else:
                    raise Exception(f"Stream {index} not opened")
                time.sleep((1 / self.fps))  # wait time
            except Exception as e:
                print(e)
                print(f"Attempting to reconnect to stream {index}...")
                cap.release()  # close the current connection
                time.sleep(1)  # wait a bit before reconnecting
                cap = cv2.VideoCapture(self.sources[index]) # try to reopen the connection
